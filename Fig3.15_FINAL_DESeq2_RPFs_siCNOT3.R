...
most_abundant_transcripts <- read_csv(file = file.path(parent_dir, "Analysis/most_abundant_transcripts/most_abundant_transcripts_IDs_siCNOT3.csv"))
...
sample_info <- data.frame(row.names = RPF_sample_names,
                          condition = factor(rep(c("NTC", "siCNOT3"), 5)),
...
# sink(file = file.path(parent_dir, "Analysis/DESeq2_output", paste0("RPFs_", treatment, "_DEseq2_summary_siCNOT3.txt")))
# summary(res)
# sink()
...
write_csv(DEseq2_output, file = file.path(parent_dir, "Analysis/DESeq2_output", paste0("FINAL_RPFs_", treatment, "_DEseq2_apeglm_LFC_shrinkage.csv")))
...
write_csv(normalised_counts, file = file.path(parent_dir, "Analysis/DESeq2_output", paste0("FINAL_RPFs_", treatment, "_normalised_counts.csv")))
...
png(filename = file.path(parent_dir, "plots/PCAs", paste0(treatment, "_RPFs_batch_corrected_PCA_siCNOT3.png")), width = 400, height = 350)
ggplot(pcaData, aes(PC1, PC2, color=condition, label=replicate)) +
  ...
  theme(axis.title = element_text(size = 18),
        ...
        legend.title = element_blank(),
        plot.title = element_text(size = 20, face = "bold", hjust = 0.5))+
  ggtitle("siCNOT3/siRPS25 RPFs") +
  publication_theme()
dev.off()

# png(filename = file.path("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/riboseq_RPFs_siCNOT3.png"), width = 1250, height = 1250, res = 300)
# ggplot(pcaData, aes(PC1, PC2, color=condition, label=replicate)) +
#   geom_point(size=3) +
#   geom_text(size = 6, nudge_x = 2, vjust=1, show.legend = F)+
