...
bin_data <- function(df, region_lengths, region_cutoffs = c(50,300,50), bins = c(25,50,25)) {
  ...
  sample <- df$condition[[1]]
  
  print(paste("filtering transcripts replicate", replicate, sample))
  ...
  print(paste("binning 5'UTR replicate", replicate, sample))
  ...
  print(paste("binning CDS replicate", replicate, sample))
  ...
  print(paste("binning 3'UTR replicate", replicate, sample))
...
splice_single_nt <- function(df, region_lengths, codons = 50, UTR_nts = 48, region_cutoffs = c(50,300,50)) {
  ...
  print(paste("splicing 5'UTR replicate", replicate, sample))
  ...
  print(paste("splicing CDS replicate", replicate, sample))
  ...
  print(paste("splicing 3'UTR replicate", replicate, sample))
...
summarise_data <- function(df, value, grouping) {
  df %>%
    dplyr::rename(value = value) %>% 
    dplyr::rename(grouping = grouping) %>%
...
plot_binned_lines <- function(df, SD = F, conditions = NULL, mylabels = NULL, colours = NULL, CDS_only = F) {
...
plot_binned_all_replicates <- function(alist, conditions = c(control, treatment), mylabels = NULL, colours = NULL) {
  ...
  df[df$region == "UTR3",] %>%
    ...
    ylim(ylims)+
    UTR3_theme +
    theme(legend.margin = unit(0.5, "cm"))-> UTR3_plot
...
plot_positional_lines <- function(df, SD = F, conditions = NULL, mylabels = NULL, colours = NULL, title = NULL) {
  ...
  df %>%
    ...
    theme(axis.line.y = element_line(),
          ...
          axis.title.y = element_text(angle=90,vjust =2))-> positional_plot
  
  return(positional_plot)
}

plot_single_nt_lines <- function(df, SD = F, conditions = NULL, mylabels = NULL, colours = NULL, plot_ends = F, title = NULL) {
...
plot_subset <- function(IDs, subset, sub_dir,
                        ...
                        plot_replicates = F, plot_delta = T, SD = T, paired_data = T, mylabels = NULL) {
  ...
  if (plot_binned == T) {
    ...
    #plot lines
    subset_binned_line_plots <- plot_binned_lines(summarised_subset_binned, 
                                                  SD = SD, 
    ...
    png(filename = file.path(parent_dir, 
        ...
        height = 200)
    grid.arrange(
      subset_binned_line_plots[[1]], 
      subset_binned_line_plots[[2]], 
      subset_binned_line_plots[[3]], 
      nrow = 1, 
      widths = c(1,2,1.5))
    dev.off()
    ...
    if (plot_delta == T) {
      ...
      subset_binned_delta_data <- calculate_binned_delta(subset_binned_list, 
                                                         value = binned_value, 
      ...
      png(filename = file.path(parent_dir, 
          ...
          height = 200)
      grid.arrange(
        subset_binned_delta_plots[[1]], 
        subset_binned_delta_plots[[2]], 
        subset_binned_delta_plots[[3]], 
        nrow = 1, 
        widths = c(1,2,1))
      dev.off()
    }
    ...
    if (plot_replicates == T) {
      ...
      png(filename = file.path(parent_dir, 
                               ...
                               paste(treatment, 
                                     subset, 
                                     binned_value, 
                                     "lines all replicates.png")), 
          width = 1000, 
          height = 200)
      grid.arrange(
        subset_binned_line_plots_all_replicates[[1]], 
        subset_binned_line_plots_all_replicates[[2]], 
        subset_binned_line_plots_all_replicates[[3]], 
        nrow = 1, 
        widths = c(1,2,1.5))
      dev.off()
    }
  ...
  if (plot_single_nt == T) {
    ...
    #summarise
    summarised_subset_single_nt_list <- lapply(subset_single_nt_list, 
                                               summarise_data, 
                                               value = single_nt_value, 
                                               grouping = "window")
    ...
    #plot lines
    subset_single_nt_line_plots <- plot_single_nt_lines(summarised_subset_single_nt, 
                                                        SD = SD,
                                                        conditions = c(control, treatment),
                                                        mylabels = mylabels)
    ...
    png(filename = file.path(parent_dir, 
                             "plots/binned_plots", 
                             sub_dir, 
                             paste(treatment, subset, single_nt_value, "lines.png")), 
        width = 1300, 
        height = 300)
    grid.arrange(
      subset_single_nt_line_plots[[1]], 
      subset_single_nt_line_plots[[2]], 
      subset_single_nt_line_plots[[3]], 
      subset_single_nt_line_plots[[4]], 
      nrow = 1, widths = c(1,2,2,1.5))
    dev.off()
    
    if (plot_delta == T) {
      #calculate and plot delta
      subset_single_nt_delta_data <- calculate_single_nt_delta(subset_single_nt_list, 
                                                               value = single_nt_value, 
                                                               control = control, 
                                                               treatment = treatment, 
                                                               paired_data = paired_data)
      ...
      png(filename = file.path(parent_dir, "plots/binned_plots", 
                               sub_dir, paste(treatment, 
                                              subset, single_nt_value, "delta.png")), 
          width = 1300, height = 200)
      grid.arrange(
        subset_single_nt_delta_plots[[1]], 
        subset_single_nt_delta_plots[[2]], 
        subset_single_nt_delta_plots[[3]], 
        subset_single_nt_delta_plots[[4]], 
        nrow = 1, widths = c(1,2,2,1))
      dev.off()
    }
  }
  ...
  if (plot_positional == T) {
    ...
    summarised_positional_subset_binned_list <- lapply(subset_positional_binned_list, 
                                                       summarise_data, 
                                                       value = binned_value, 
                                                       grouping = "bin")
    ...
    subset_positional_line_plots <- plot_positional_lines(df = summarised_positional_subset_binned, 
                                                          SD = SD, 
                                                          conditions = c(control, treatment),
                                                          mylabels = mylabels)
    ...
    png(filename = file.path(parent_dir, 
                             "plots/binned_plots", 
                             sub_dir, paste(treatment, subset, "binned positional lines.png")), 
        width = 500, 
        height = 200)
    ...
    if (plot_delta == T) {
      subset_binned_positional_delta <- calculate_positional_delta(subset_positional_binned_list, 
                                                                   control = control, treatment = treatment,
                                                                   #mylabels = mylabels,
                                                                   paired_data = paired_data)
      ...
      png(filename = file.path(parent_dir, 
                               "plots/binned_plots", 
                               sub_dir, 
                               paste(treatment, subset, "binned positional delta.png")), 
