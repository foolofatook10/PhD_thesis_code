...
replican <- unlist(lapply(str_split(data_to_plt$sample, " "), function(x){x[[1]]}))
condytion <- unlist(lapply(str_split(data_to_plt$sample, " "), function(x){x[[2]]}))

data_to_plt$replicate <- replican
data_to_plt$condition1 <- condytion

data_to_plt <- 
data_to_plt %>% mutate(condition1 = case_when(condition1 == "CNOT3" ~ "CNOT3 IP'd RPFs",
                                              condition1 == "Tot" ~ "Total RPFs"))
...
data_to_plt %>% 
    ...
    ylab("% Counts")+
    scale_x_continuous(breaks = seq(min(lengths), max(lengths),2))+
    ggtitle(paste0(unique(data_to_plt$replicate), ":\n", unique(data_to_plt$condition1))) +
    theme(legend.position = "none")-> region_perc_plot
  
  png(filename = file.path(parent_dir, paste0("plots/summed_counts/", sample, "_perc_region_counts.png")), width = 1500, height = 1250, res = 300)
