...
# read in arabidopsis genome table
plant <- as_tibble(fread("\\\\data.beatson.gla.ac.uk/data/R11/James/sequences/arabidopsis.csv")) %>% 
  select(gene, nucleotide_sequence_CDS, signal_sequence)

# read in master table
#master <- as_tibble(fread(file = "\\\\data.beatson.gla.ac.uk/data/R11/James/sequences/mouse_master.csv", header = T))

# select the longest transcript per gene
longest <- plant %>% 
  mutate(CDS_length = nchar(plant$nucleotide_sequence_CDS)) %>%
  group_by(gene) %>%
...
#ENSMUSTs <- unique(features$ENSMUST)
...
#combined <- inner_join(features, longest, by = c("ENSMUSG","ENSMUST"))
...
# Name each element of the list by its ENSMUST tag
...
codonsN_table_long <- 
  ...
  inner_join(plant %>% select(gene, signal_sequence), by = "gene")
...
# tiff(file.path("GC3_mouse.tiff"), height = 5, width = 5, units = "in", res = 200)
# print(GC3)
# dev.off()
...
background <- lines_processed %>% filter(leucyl == "other" & position != 1 & codon != "ttc")
background_ttc <- lines_processed %>% filter(codon == "ttc" & position != 1)
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/arabidopsis_thaliana.png",
    res = 300,
    height = 1000,
    width = 1500)
print(final_gplot)
...
lines_processed <- 
lines_processed%>% 
  mutate(new_codon = case_when(codon == "ctt" ~ "ctt",
                            codon == "cta" ~"cta",
                            codon == "ctg" ~ "ctg",
                            codon == "tta" ~ "tta",
                            codon == "ttg" ~"ttg",
...
# make leucyl codons column a factor
lines_processed$new_codon <- factor(lines_processed$new_codon, levels = c("ctg", "ctc", "ctt", "ttg", "cta", "tta", "ttc", "other"))

# plot the leucyl codon usage for transcripts with and without a signal sequence
lines_gplot <- 
  lines_processed %>% 
  ggplot(aes(x = position, y = percent, group = codon, color = new_codon)) +
  geom_line(size = 1) +
  scale_color_manual(values = c("#377EB8", "#984EA3","#4DAF4A", "#BF5B17", "#FF7F00", "#F0027F","#E41A1C", "#999999")) +
  ylim(0,20) +
  xlab("Codon Position") +
  ylab("Percent") +
  ggtitle("Plant transcripts") +
  facet_wrap(~tag) +
  publication_theme() +
...
tiff(file.path("CTGenrichment_plant.tiff"), height = 3, width = 6, units = "in", res = 500)
print(lines_gplot)
dev.off()
