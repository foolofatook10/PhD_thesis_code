...
CPMS <-
  CPMS %>%
  filter(MD30!= "D30") %>%
  select(replicate, transcript, codon, CPM = summed_CPM_codon, IP)
...
# Number of unique transcripts (one transcript per gene) for each of the classes represented in the CNOT3 SelRP dataset
...
png(file.path(thesis_figures_path, "TMs_in_CNOT3_SelRP_TM_lengths.png"),
...
transcripts_considered <- all_data
...
confidence_plot <- 
  ...
  scale_y_continuous(limits = c(0,NA), breaks = c(seq(0,1000,200))) +
  scale_x_continuous(limits = c(-21, 100),
                     breaks = seq(-20, 100, 20),
                     labels = c("-20", "0", "+20", "+40", "+60", "+80", "+100")) +
  geom_vline(xintercept = 0, lty = "longdash", size = 1, colour = "black") +
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/CNOT3_SelRP_MultipassT3_TM_domains.png", 
    res = 300, 
    height = 2250, 
    width = 1500)
...
all_stats_data %>%
  ...
  group_modify(~ {
    test <- t.test(.x$CNOT3, .x$Tot, paired = TRUE)
...
write.csv(stats_data_final, file.path(thesis_figures_path, "CNOT3SelRP_TM_domain_stats_all.csv"))
