...
home <- "\\\\data.beatson.gla.ac.uk/data/R11/James/CNOT3_Sel_RiboSeq/FINAL/CPMs"

DDX6 <- 
  as_tibble(fread(file.path(home, "DDX6_CDS_CPMs.csv"))) %>% 
  mutate(sample = "DDX6", condition = factor(case_when(condition == "DDX6" ~ "IP",
                                                       .default = condition)))
...
DDX6 %>% pull(transcript) %>% unique() %>% length()
...
CDS_sigseqs <- 
  DDX6 %>% filter(transcript %in% over100codons) %>% 
...
CPMS <-
  CDS_sigseqs %>% 
  select(replicate, condition, transcript, signal_sequence, codon, CPM)

# consider only first 30 codons
CPMS_processed <- 
  CPMS %>%
  ungroup() %>% 
  group_by(replicate, condition, signal_sequence, transcript) %>%
  slice_head(n = -1) %>% 
  slice_head(n = 500) %>% 
  ungroup()
...
CPMs_replicates <- 
  CPMS_processed %>% 
  group_by(replicate, condition, signal_sequence, codon) %>% 
...
CPMs_average <- 
  CPMS_processed %>% 
  group_by(condition, signal_sequence, codon) %>% 
  summarise(mean_CPM = mean(CPM)) %>% 
  ggplot(aes(x = codon, y = mean_CPM, colour = condition))+
  geom_line() +
