...
normalised_CPMs <- fread("\\\\data.beatson.gla.ac.uk/data/R11/James/CNOT3_Riboseq/RPF_nCPMs_siCNOT3.csv")
...
normalised_CPMs_CNOT3 <- normalised_CPMs %>% filter(KD %in% c("siCNOT3", "NTC"))

transcripts_per_condition <- lapply(split(normalised_CPMs_CNOT3, normalised_CPMs_CNOT3$id), function(x){x %>% pull(transcript)})

final_transcripts <- Reduce(intersect, transcripts_per_condition)

normalised_CPMs_CNOT3 <- 
  normalised_CPMs_CNOT3 %>% 
  filter(transcript %in% final_transcripts)

filt_transcripts <- intersect(unique(normalised_CPMs_CNOT3$transcript), features %>% filter(cds_length > 300) %>% pull(ENST))

normalised_CPMs_CNOT3 <- 
  normalised_CPMs_CNOT3 %>% filter(transcript %in% filt_transcripts)

normalised_CPMs_CNOT3$KD <- factor(normalised_CPMs_CNOT3$KD)
...
deltas <- 
  ...
  mutate(siCNOT3_delta = siCNOT3 - NTC) %>% 
  select(replicate, transcript, codon, siCNOT3_delta)

table(is.na(deltas$siCNOT3_delta))
...
final <- 
  ...
  summarise(
    n = n(),
    mean_delta = mean(siCNOT3_delta),
    sd_delta = sd(siCNOT3_delta, na.rm = TRUE),
...
deltas %>% 
  ...
  summarise(mean_delta = mean(siCNOT3_delta)) %>% 
  ungroup() %>% 
  group_by(transcript) %>% 
  slice_head(n = -1)
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/siCNOT3_SPs.png",
