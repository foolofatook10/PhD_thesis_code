...
setwd("\\\\data.beatson.gla.ac.uk/data/R11/James/CNOT3_Sel_RiboSeq/FINAL/CPMs")
save_path = "\\\\data.beatson.gla.ac.uk/data/R11/James/CNOT3_Sel_RiboSeq/FINAL/plots/signal_sequences"

# read in CPMs
CDS <- as_tibble(fread("CDS_CPMs.csv")) %>% filter(MD30 != "D30")

# read in features
features <- as_tibble(fread("\\\\data.beatson.gla.ac.uk/data/R11/James/sequences/features.csv", 
                            header = T, 
                            drop = "V1"))
...
signal_sequences <- 
  features %>% 
  select(ENST, signal_sequence)

CDS %>% pull(transcript) %>% unique() %>% length()
...
CDS %>% 
  select(transcript) %>% unique() %>% 
  filter(transcript %in% over100codons) %>% 
  inner_join(signal_sequences, by = c("transcript" = "ENST")) %>% 
...
# sanity check
# CDS_sigseqs %>% 
#   dplyr::group_by(replicate,MD30,IP, transcript) %>% 
#   summarise(total = round(sum(perc_counts_across_CDS))) %>% 
#   pull(var = total) %>% 
#   table()

CDS_sigseqs %>% select(-MD30) -> CDS_sigseqs

CPMS <-
  CDS_sigseqs %>% 
  select(replicate, IP, transcript, signal_sequence, codon, summed_CPM_codon)

# percentages <- 
#   CDS_sigseqs %>% 
#   select(replicate, IP, transcript, signal_sequence, codon, perc_counts_across_CDS) %>% 
#   dplyr::rename(percentages = perc_counts_across_CDS)
...
# features %>% 
#   filter(ENST %in% (CPMS %>% pull(transcript) %>% unique())) %>% 
#   group_by(cds_length) %>% 
#   tally() %>% 
#   ggplot(aes(x = cds_length/3, y = n)) +
#   geom_col()

# max_cds_length <- 
# features %>% 
#   select(ENST, cds_length) %>% 
#   mutate(cds_length_codon = (cds_length/3 - 1))
# 
# CPMs_int <- 
# CPMS %>% 
#   inner_join(max_cds_length, by = c("transcript" = "ENST"))
# 
# CPMs_int_split <- 
# split(CPMs_int, CPMs_int$transcript)
# 
# CPMs_int_split
# filter_max_codon <- function(x){
#   
#   max_codon <- x$cds_length_codon[[1]]
#   return(x %>% filter(codon <= max_codon))
#   
#   }
# 
# CPMs <- 
# do.call("rbind", lapply(CPMs_int_split, filter_max_codon))
...
CPMs_replicates <- 
  ...
  summarise(mean_CPM = mean(summed_CPM_codon)) %>% 
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/CNOT3SelRP_SPs.png",
