...
leishmania <- 
  read.fasta("C:/Users/Jettles/OneDrive - University of Glasgow/Documents/leishmania/Leishmania_major.ASM272v2.cds.all.fa", whole.header = T)

sequences <- 
  unlist(getSequence(leishmania, as.string = T))
names <- unlist(getName.list(leishmania))
...
get_genes <- function(x){
  ...
  str_extract(x, pattern = "LMJF_\\d+_\\d+")
  #lapply(str_split(x, " "), function(x){x[[1]]})
}
...
leishmania_sequences_atg <- 
  leishmania_sequences %>% 
  filter(str_detect(sequence, pattern = "^atg"))

leishmania_sequences_stops <- 
  leishmania_sequences_atg %>% 
  filter(str_detect(sequence, pattern = "(taa|tag|tga)$"))

leishmania_sequences_correct_length <- 
  leishmania_sequences_stops %>% 
...
write.fasta(sequences_split[1:5000], names = names_fasta[1:5000], file.out = "C:/Users/Jettles/OneDrive - University of Glasgow/Documents/leishmania_1_5000.fa")
write.fasta(sequences_split[5001:8158], names = names_fasta[5000:8158], file.out = "C:/Users/Jettles/OneDrive - University of Glasgow/Documents/leishmania_5000_10000.fa")



fyles <- paste0("C:/Users/Jettles/OneDrive - University of Glasgow/Documents/leishmania/output_matureLM", 
                c("1", "2"),
                ".fasta")
