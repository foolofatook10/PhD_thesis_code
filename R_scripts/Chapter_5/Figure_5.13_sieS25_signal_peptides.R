...
normalised_CPMs <- fread("\\\\data.beatson.gla.ac.uk/data/R11/James/CNOT3_Riboseq/RPF_nCPMs_siRPS25.csv")
...
normalised_CPMs_RPS25 <- normalised_CPMs %>% filter(KD %in% c("siRPS25", "NTC"))

transcripts_per_condition <- lapply(split(normalised_CPMs_RPS25, normalised_CPMs_RPS25$id), function(x){x %>% pull(transcript)})

final_transcripts <- Reduce(intersect, transcripts_per_condition)

normalised_CPMs_RPS25 <- 
  normalised_CPMs_RPS25 %>% 
  filter(transcript %in% final_transcripts)

filt_transcripts <- intersect(unique(normalised_CPMs_RPS25$transcript), features %>% filter(cds_length > 300) %>% pull(ENST))

normalised_CPMs_RPS25 <- 
normalised_CPMs_RPS25 %>% filter(transcript %in% filt_transcripts)
...
normalised_CPMs_RPS25$KD <- factor(normalised_CPMs_RPS25$KD)
...
deltas <- 
  ...
  mutate(siRPS25_delta = siRPS25 - NTC) %>% 
  select(replicate, transcript, codon, siRPS25_delta)

table(is.na(deltas$siRPS25_delta))
...
final <- 
  ...
  summarise(
    n = n(),
    mean_delta = mean(siRPS25_delta),
    sd_delta = sd(siRPS25_delta, na.rm = TRUE),
...
mean_deltas <- 
  ...
  summarise(mean_delta = mean(siRPS25_delta)) %>% 
  ungroup() %>% 
  group_by(transcript) %>% 
  slice_head(n = -1)
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/sieS25_SPs.png",
