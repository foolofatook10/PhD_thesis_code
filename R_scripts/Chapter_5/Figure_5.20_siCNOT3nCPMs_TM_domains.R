...
CPMS <- as_tibble(fread("\\\\data.beatson.gla.ac.uk/data/R11/James/CNOT3_Riboseq/RPF_nCPMs_siCNOT3.csv"))
...
CPMS <-
  CPMS  %>%
  select(replicate, transcript, codon, CPM, KD)

# calculate the mean CPM at each codon position within each transcript
mean_CPMs <-
  CPMS %>%
  group_by(transcript, codon, KD) %>%
  summarise(
    #n = n(),
    mean_CPM = mean(CPM))
...
mean_CPMs_split <- split(mean_CPMs, mean_CPMs$KD)
...
all_data <-
  ...
  select(transcript, codon, KD, mean_CPM, region, rel_position, codon_sequence, ID, class, tag)
...
very_high_CPMs <- 
  ...
  pull(transcript) %>% 
  unique() #Identifies 82 transcripts
...
# Number of unique transcripts (one transcript per gene) for each of the classes represented in the siCNOT3 dataset
...
# Split the data
CNOT3_TM_data_split <- 
  split(CNOT3_TM_data, CNOT3_TM_data$tag)

# understand the lengths of all TM domains
TM_lengths_gplot <- 
  unlist(lapply(CNOT3_TM_data_split, function(x){(min(x$rel_position) -1) * -1})) %>% 
  table() %>% 
  as.data.frame() %>% 
...
png(file.path(thesis_figures_path, "TMs_in_siCNOT3_TM_lengths.png"),
...
single_pass_filt <- c("type1", "type2", "type3")
multi_pass_filt <- c("multipassT1", "multipassT2", "multipassT3")

transcripts_considered <- all_data %>% filter(class== "multipassT2")

# Calculate mean CPM
test4 <- transcripts_considered %>% 
  dplyr::rename(CPM = mean_CPM) %>% 
  group_by(region, rel_position, KD) %>%
...
actual_plot <- 
  test4 %>% 
  filter(rel_position >=-21) %>% 
  mutate(KD = factor(KD, levels = c("siCNOT3", "NTC"))) %>% 
  ggplot(aes(x = rel_position, y = mean_CPM, colour = KD, group = KD))   +
  ...
  geom_rect(aes(xmin=0, xmax=100, ymin=-Inf, ymax=Inf),
            ...
            inherit.aes = F)+
  scale_color_manual(values = c("#0099B4FF", "#FFC20A")) +
  scale_fill_manual(
    values = c("#0099B4FF", "#FFC20A")
  ) +
...
confidence_plot <- 
  ...
  scale_y_continuous(limits = c(0,NA), breaks = c(seq(0,2500,200))) +
  scale_x_continuous(limits = c(-21, 100),
                     breaks = seq(-20, 100, 20),
                     labels = c("-20", "0", "+20", "+40", "+60", "+80", "+100")) +
  geom_vline(xintercept = 0, lty = "longdash", size = 1, colour = "black") +
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/siCNOT3_multipassT2_TM_domains.png", 
    res = 300, 
    height = 2250, 
    width = 1500)
...
generate_aucs <- function(filter_term){
  ...
  if(filter_term== "all_TMs"){
    ...
    stats_analysis <- 
      ...
      dplyr::rename(CPM = mean_CPM) %>%
      ungroup() %>% 
      select(region, KD, CPM, rel_position, tag)%>% 
      group_by(region, tag, KD) %>% 
...
stats_data_final <- 
  ...
  select(region, subgroup, tag, KD, auc) %>%
  pivot_wider(
    names_from = KD,
    values_from = auc
  ) %>%
  ...
  group_modify(~ {
    test <- t.test(.x$siCNOT3, .x$NTC, paired = TRUE)
...
write.csv(stats_data_final, file.path(thesis_figures_path, "siCNOT3_TM_domain_stats_all.csv"))
