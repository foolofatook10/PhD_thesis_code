...
canis <- 
read.fasta("C:/Users/Jettles/OneDrive - University of Glasgow/Documents/Canis_lupus_familiaris.ROS_Cfam_1.0.cds.all.fa", whole.header = T)

sequences <- 
  unlist(getSequence(canis, as.string = T))
names <- unlist(getName.list(canis))

get_transcripts <- function(x){
  
  str_extract(x, pattern = "^ENSCAFT\\d+\\.\\d+")
  
}

get_genes <- function(x){
  
  str_extract(x, pattern = "ENSCAFG\\d+\\.\\d+")
}
...
canis_sequences_stops <- 
  canis_sequences_atg %>% 
  filter(str_detect(sequence, pattern = "(taa|tag|tga)$"))

canis_sequences_correct_length <- 
  canis_sequences_stops %>% 
...
write.fasta(sequences_split[1:5000], names = names_fasta[1:5000], file.out = "C:/Users/Jettles/OneDrive - University of Glasgow/Documents/Canis_lupus_1_5000.fa")
write.fasta(sequences_split[5001:10000], names = names_fasta[5001:10000], file.out = "C:/Users/Jettles/OneDrive - University of Glasgow/Documents/Canis_lupus_5000_10000.fa")
write.fasta(sequences_split[10001:15000], names = names_fasta[10001:15000], file.out = "C:/Users/Jettles/OneDrive - University of Glasgow/Documents/Canis_lupus_10000_15000.fa")
write.fasta(sequences_split[15001:18378], names = names_fasta[15001:18378], file.out = "C:/Users/Jettles/OneDrive - University of Glasgow/Documents/Canis_lupus_15000_20000.fa")

fyles <- paste0("\\\\data.beatson.gla.ac.uk/data/R11/James/sequences/raw_data/canis_lupus/processed_entries", 
                c("_1", "_2", "_3", "_4"),
                ".fasta")
...
write.csv(transcripts_annotated, "\\\\data.beatson.gla.ac.uk/data/R11/James/sequences/canis_lupis.csv")
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/canis_lupus.png",
    res = 300,
    height = 1000,
    width = 1500)
print(final_gplot)
