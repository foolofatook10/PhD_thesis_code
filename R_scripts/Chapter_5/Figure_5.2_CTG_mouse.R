...
features <- as_tibble(fread("\\\\data.beatson.gla.ac.uk/data/R11/James/sequences/features_mouse.csv")) %>%
  select(ENSMUSG, ENSMUST, signal_sequence)
...
names(my_lyst) <- pull(combined, var = ENSMUST)
...
transcripts_post_filtering <- 
  combined %>% 
  filter(ENSMUST %in% names(my_lyst)) %>% 
...
codonsN_table <- 
  codonsN_table %>% rownames_to_column("ENSMUST")
...
saveRDS(GC3, file = "\\\\data.beatson.gla.ac.uk/data/JETTLES/CTG_codons/bioinformatic_analysis/scripts/ggplot_objects/mouseGC3.rdata")

 tiff(file.path("GC3_mouse.tiff"), height = 4, width = 5, units = "in", res = 500)
 print(GC3)
 dev.off()
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/mus_musculus.png",
    res = 300,
    height = 1000,
    width = 1500)
print(final_gplot)
...
tiff(file.path("CTGenrichment_mouse.tiff"), height = 3, width = 6, units = "in", res = 500)
print(lines_gplot)
dev.off()
