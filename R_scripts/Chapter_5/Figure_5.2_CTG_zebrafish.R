...
zebrafish_transcriptome <- fread("\\\\data.beatson.gla.ac.uk/data/R11/James/sequences/danio_rerio.csv",
                                 header = T,
                                 drop = "V1")

zebrafish_transcriptome <- as_tibble(zebrafish_transcriptome)
...
combined <- zebrafish_transcriptome
...
names(my_lyst) <- pull(combined, var = transcript)
...
transcripts_post_filtering <- 
  combined %>% 
  filter(transcript %in% names(my_lyst)) %>% 
...
sanity_check <- codonsN_table_long %>% arrange(transcript)

# add amino acid information and signal sequence information to the table
codonsN_table_long <- 
  codonsN_table_long %>%
  inner_join(codons %>% select(codon, AA), by = "codon") %>%
  inner_join(combined %>% select(transcript, signal_sequence), by = "transcript")
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/danio_rerio.png",
    res = 300,
    height = 1000,
    width = 1500)
print(final_gplot)
...
tiff(file.path("CTGenrichment_zebrafish.tiff"), height = 3, width = 6, units = "in", res = 500)
print(lines_gplot)
dev.off()
