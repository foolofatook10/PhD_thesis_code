...
# read in fly table
fly_transcriptome <- fread("\\\\data.beatson.gla.ac.uk/data/R11/James/sequences/drosophila_melanogaster.csv",
                                 header = T,
                                 drop = "V1")

fly_transcriptome <- as_tibble(fly_transcriptome)
...
saveRDS(GC3, file = "\\\\data.beatson.gla.ac.uk/data/JETTLES/CTG_codons/bioinformatic_analysis/scripts/ggplot_objects/flyGC3.rdata")

 tiff(file.path("GC3_fly.tiff"), height = 4, width = 5, units = "in", res = 200)
 print(GC3)
 dev.off()
...
final_gplot <- 
  ...
  ggtitle("Drosophila melanogaster") +
  facet_wrap(~tag) +
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/drosophila_melanogaster.png",
    res = 300,
    height = 1000,
    width = 1500)
print(final_gplot)
...
tiff(file.path("CTGenrichment_fly.tiff"), height = 3, width = 6, units = "in", res = 500)
print(lines_gplot)
dev.off()
