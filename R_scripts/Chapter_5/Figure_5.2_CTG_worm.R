...
# read in worm table
worm_transcriptome <- fread("\\\\data.beatson.gla.ac.uk/data/R11/James/sequences/caenorhabditis_elegans.csv",
                           header = T,
                           drop = "V1")

worm_transcriptome <- as_tibble(worm_transcriptome)
...
final_gplot <- 
  ...
  ggtitle("Caenorhabditis elegans") +
  facet_wrap(~tag) +
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/C_elegans.png",
    res = 300,
    height = 1000,
    width = 1500)
print(final_gplot)
...
# make leucyl codons column a factor
lines_processed$leucyl <- factor(lines_processed$leucyl, levels = c("ctg", "ctc", "ctt", "ttg", "cta", "tta", "ttc", "ttt", "att", "other"))
...
lines_gplot <- 
  ...
  geom_line(size = 1) +
  scale_color_manual(values = c("#377EB8", "#984EA3","#4DAF4A", "#BF5B17", "#FF7F00", "#F0027F", "black", "red", "blue",  "#999999")) +
  ylim(0,20) +
  xlab("Codon Position") +
  ylab("Percent") +
  ggtitle("Worm transcripts") +
  facet_wrap(~tag) +
  publication_theme() +
...
#   ggplot(aes(x = position, y = percent, group = codon)) +
#   geom_line(size = 1) +
...
tiff(file.path("CTGenrichment_worm.tiff"), height = 3, width = 6, units = "in", res = 500)
print(lines_gplot)
dev.off()
