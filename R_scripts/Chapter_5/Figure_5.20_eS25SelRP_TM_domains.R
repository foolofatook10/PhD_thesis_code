...
CPMS <- as_tibble(fread("\\\\data.beatson.gla.ac.uk/data/R11/external_sequencing_data/rps25_sel-RiboSeq/CPMs/CDS_CPMs.csv"))
...
CPMS <-
  CPMS %>%
  #filter(MD30!= "D30") %>%
  select(replicate, transcript, codon, CPM = mean_CPM_codon, IP)
...
TM_domains <- 
  read_csv("C:/Users/Jettles/OneDrive - University of Glasgow/Documents/TM_domains_FINAL_MOUSE.csv") %>% 
...
very_high_CPMs <- 
  ...
  pull(transcript) %>% 
  unique() #Identifies 9 transcripts
...
# Number of unique transcripts (one transcript per gene) for each of the classes represented in the eS25 SelRP dataset
...
# Split the data
es25_TM_data_split <- 
  split(eS25_TM_data, eS25_TM_data$tag)

# understand the lengths of all TM domains
TM_lengths_gplot <- 
  unlist(lapply(es25_TM_data_split, function(x){(min(x$rel_position) -1) * -1})) %>% 
  table() %>% 
  as.data.frame() %>% 
...
png(file.path(thesis_figures_path, "TMs_in_eS25_SelRP_TM_lengths.png"),
...
actual_plot <- 
  ...
  geom_rect(aes(xmin=0, xmax=100, ymin=-Inf, ymax=Inf),
            ...
            inherit.aes = F)+
  scale_color_manual(values = c("#984EA3", "#FFC20A")) +
  scale_fill_manual(
    values = c("#984EA3", "#FFC20A")
  ) +
...
confidence_plot <- 
  ...
  scale_y_continuous(limits = c(0,NA), breaks = c(seq(0,1000,20))) +
  scale_x_continuous(limits = c(-21, 100),
                     breaks = seq(-20, 100, 20),
                     labels = c("-20", "0", "+20", "+40", "+60", "+80", "+100")) +
  geom_vline(xintercept = 0, lty = "longdash", size = 1, colour = "black") +
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/eS25_SelRP_singlepassT3_TM_domains.png", 
    res = 300, 
    height = 2250, 
    width = 1500)
...
stats_data_final <- 
  ...
  group_modify(~ {
    test <- t.test(.x$rps25, .x$Total, paired = TRUE)
...
write.csv(stats_data_final, file.path(thesis_figures_path, "eS25SelRP_TM_domain_stats_all.csv"))
