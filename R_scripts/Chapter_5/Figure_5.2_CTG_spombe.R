...
# read in pombe table
pombe_transcriptome <- fread("\\\\data.beatson.gla.ac.uk/data/R11/James/sequences/schizosaccharomyces_pombe.csv",
                                 header = T,
                                 drop = "V1")

pombe_transcriptome <- as_tibble(pombe_transcriptome)
...
lines_processed$leucyl <- factor(lines_processed$leucyl, levels = c("ctg", "ctc", "ctt", "ttg", "cta", "tta", "ttt", "gct", "other"))
...
#
background <- lines_processed %>% filter(leucyl == "other" & position != 1 & codon != "ttt")
background_ttt <- lines_processed %>% filter(position != 1 & codon == "ttt")
...
#ggsci::pal_npg("nrc")(15)
#"#E64B35FF" "#4DBBD5FF" "#00A087FF" "#3C5488FF" "#F39B7FFF" "#8491B4FF"  "#DC0000FF" "#7E6148FF" "#B09C85FF"
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/s_pombe.png",
    res = 300,
    height = 1000,
    width = 1500)
print(final_gplot)
...
tiff(file.path("CTGenrichment_pombe.tiff"), height = 3, width = 6, units = "in", res = 500)
print(lines_gplot)
dev.off()
