  ...
  all2 <- 
    ...
    mutate(
      pathway = case_when(
        ...
        pathway == " PROTEIN LOCALIZATION TO ENDOPLASMIC RETICULUM" ~ "PROTEIN LOCALIZATION\n TO ENDOPLASMIC RETICULUM",
        pathway == " ESTABLISHMENT OF PROTEIN LOCALIZATION TO ENDOPLASMIC RETICULUM" ~ "ESTABLISHMENT\nOF PROTEIN LOCALIZATION\nTO ENDOPLASMIC RETICULUM",
...
DESeq2_data <- read_csv(file = file.path(parent_dir, "Analysis/DESeq2_output/FINAL_siCNOT3_merged_DESeq2.csv"))
...
save_path = "\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/gsea/siCNOT3"
...
# RPFs
png(file.path(save_path, "siCNOT3_GSEA_RPFs.png"), res = 300, height = 6500, width = 2000)
...
# mRNA
png(file.path(save_path, "siCNOT3_GSEA_Totals.png"), res = 300, height = 6500, width = 2000)
cowplot::plot_grid(cellular_component_totals,
                   bio_process_totals,
                   hallmark_totals,
                   align = "v", 
                   nrow = 3, 
...
png(file.path(save_path, "siCNOT3_GSEA_TE.png"), res = 300, height = 6500, width = 2000)
cowplot::plot_grid(cellular_component_TE,
                   bio_process_TE,
                   hallmark_TE,
                   align = "v", 
                   nrow = 3, 
...
#plot enriched pathways
png(filename = file.path(save_path, paste(treatment, "RPFs_curated.png", sep = "_")), width = 1000, height = 2000)
make_plot(fgsea_result = curated_results$RPFs, padj_threshold = padj, title = paste(treatment, "RPFs\nGSEA Curated gene sets"))
dev.off()

png(filename = file.path(save_path, paste(treatment, "totals_curated.png", sep = "_")), width = 1000, height = 2000)
make_plot(fgsea_result = curated_results$totals, padj_threshold = padj, title = paste(treatment, "Total RNA\nGSEA Curated gene sets"))
dev.off()

png(filename = file.path(save_path, paste(treatment, "TE_curated.png", sep = "_")), width = 1000, height = 1000)
make_plot2(fgsea_result = curated_results$TE, padj_threshold = padj, title = paste(treatment, "TE\nGSEA Curated gene sets"))
dev.off()
