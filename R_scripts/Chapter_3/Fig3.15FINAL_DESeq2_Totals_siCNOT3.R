...
source("common_variables_siCNOT3.R")
...
sample_info <- data.frame(row.names =  Total_sample_names,
                          condition = c(rep(c("NTC","siCNOT3"),5)), 
                          replicate = factor(c(rep(c(1,2,3,4,5), each = 2))))
...
# sink(file = file.path(parent_dir, "Analysis/DESeq2_output", paste0("Totals_", treatment, "_DEseq2_summary_siCNOT3.txt")))
# summary(res)
# sink()
...
write_csv(DEseq2_output, file = file.path(parent_dir, "Analysis/DESeq2_output", paste0("Totals_", treatment, "_DEseq2_apeglm_LFC_shrinkage_siCNOT3.csv")))
...
write_csv(normalised_counts, file = file.path(parent_dir, "Analysis/DESeq2_output", paste0("Totals_", treatment, "_normalised_counts_siCNOT3.csv")))
...
png(filename = file.path("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/riboseq_totals_siCNOT3.png"), width = 1250, height = 1250, res = 300)
