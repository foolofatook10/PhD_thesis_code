...
Total_sample_names <- Total_sample_names[!Total_sample_names == "REP3_siRPS25_Tot"]
...
sample_info <- data.frame(row.names =  Total_sample_names,
                          condition = c(rep(c("NTC","siCNOT3","siRPS25"),2), 
                                        rep(c("NTC","siCNOT3"),1),
                                        rep(c("NTC","siCNOT3","siRPS25"),2)),
                          replicate = factor(c(rep(c(1,2), each = 3),
                                        rep(c(3), each = 2),
                                        rep(c(4,5), each = 3))))
...
ggplot(pcaData, aes(PC1, PC2, color=condition, shape=replicate)) +
  ...
  theme(axis.title = element_text(size = 18),
        ...
        legend.title = element_blank(),
        plot.title = element_text(size = 20, face = "bold", hjust = 0.5))+
  ggtitle("siCNOT3/siRPS25 batch corrected")
dev.off()
