...
# read in merged data
data <- read_csv(file.path("\\\\data.beatson.gla.ac.uk/data/R11/James/CNOT3_Riboseq/Analysis/DESeq2_output/FINAL_siCNOT3_merged_DESeq2.csv")) 

data <- 
inner_join(data, most_abundant_transcripts, by = c("gene", "gene_sym")) %>% 
...
lapply(binned_list, function(x){unique(x$transcript) %>% length()})
...
summarised_binned_all %>%
  ...
  ylab("Mean nCPMs") +
  publication_theme() +
  theme(legend.title = element_blank())-> gplot_all

png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/siCNOT3_CDS_binned_plot_all_transcripts_mean.png",
    height = 1000, width = 1500, res = 300)
print(gplot_all)
dev.off()

png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/siCNOT3_legend.png",
