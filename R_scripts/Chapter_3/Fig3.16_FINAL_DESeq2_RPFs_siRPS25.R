...
sample_info <- data.frame(row.names = RPF_sample_names,
                          condition = factor(rep(c("NTC", "siRPS25"), 5)),
                          replicate = factor(rep(c("1", "2", "3", "4", "5"), each = 2)))
...
#DESeq2data <- DESeq2data[rownames(DESeq2data) != "ENSG00000118181.11",]
...
# sink(file = file.path(parent_dir, "Analysis/DESeq2_output", paste0("RPFs_", treatment, "_DEseq2_summary_siRPS25.txt")))
# summary(res)
# sink()
...
write_csv(DEseq2_output, file = file.path(parent_dir, "Analysis/DESeq2_output", paste0("FINAL_RPFs_", treatment, "_DEseq2_apeglm_LFC_shrinkage_contains_eS25.csv")))
...
write_csv(normalised_counts, file = file.path(parent_dir, "Analysis/DESeq2_output", paste0("RPFs_", treatment, "_normalised_counts_siRPS25.csv")))

#plot PCA----
pcaData <- plotPCA(ntd, intgroup=c("condition", "replicate"), returnData=TRUE)
...
mat <- assay(ntd)
mat <- limma::removeBatchEffect(mat, ntd$replicate)
assay(ntd) <- mat
...
png(filename = file.path(parent_dir, "plots/PCAs", paste0(treatment, "_RPFs_batch_corrected_PCA_siRPS25.png")), width = 400, height = 350)
ggplot(pcaData, aes(PC1, PC2, color=condition, label=replicate)) +
  ...
  theme(axis.title = element_text(size = 18),
        ...
        legend.title = element_blank(),
        plot.title = element_text(size = 20, face = "bold", hjust = 0.5))+
  ggtitle("siRPS25/siRPS25 RPFs") +
  publication_theme()
dev.off()

png(filename = file.path("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/riboseq_RPFs_siRPS25.png"), width = 1250, height = 1250, res = 300)
ggplot(pcaData, aes(PC1, PC2, color=condition, label=replicate)) +
  geom_point(size=3, alpha = 0.75) +
  ggrepel::geom_text_repel(size = 6, vjust=1, show.legend = F) +
  #geom_text(, position = position_jitterdodge())+
