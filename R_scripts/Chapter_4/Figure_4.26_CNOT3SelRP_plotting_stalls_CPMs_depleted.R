...
depleted <- codons_deseq %>% filter(log2FoldChange < 0 & padj < 0.05)


extract_regions <- function(a){
  
  ENST <- depleted[a,]$transcript 
  
  print(a)
  
  position <- depleted[a,]$codon
...
test <- lapply(1:nrow(depleted), extract_regions)
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/codons_deseq_codons_per_position_depleted.png", res = 300, height = 1000, width = 1000)
