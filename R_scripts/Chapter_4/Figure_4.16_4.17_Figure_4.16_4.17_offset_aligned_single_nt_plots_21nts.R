...
for (sample in RPF_sample_names) {
  ...
  #read in counts csv
  df <- read_csv(file = file.path(parent_dir, "Counts_files/csv_files", 
                                  paste0(sample, "_pc_final_21nts_counts.csv")))
...
for (sample in RPF_sample_names) {
  ...
  png(filename = file.path(parent_dir, "plots/offset_aligned_single_nt_plots", paste(tit, "offset aligned single nt plot_21nts.png")), res = 300, width = 5500, height = 1250)
...
for (sample in RPF_sample_names) {
  ...
  png(filename = file.path(parent_dir, "plots/offset_aligned_single_nt_plots", paste(sample, "offset aligned single nt plot 21nts.png")), width = 1300, height = 300)
  grid.arrange(UTR5_end_plot, CDS_start_plot, CDS_end_plot, UTR3_start_plot, nrow = 1, widths = c(1,2,2,1), top = textGrob(paste(sample),gp=gpar(fontsize=20,font=3)))
  dev.off()
}
...
png(filename = file.path(parent_dir, "plots/offset_aligned_single_nt_plots/all samples offset aligned single nt plot M 21nts.png"), width = 1300, height = 300)
grid.arrange(UTR5_end_plot, CDS_start_plot, CDS_end_plot, UTR3_start_plot, nrow = 1, widths = c(1,2,2,1),
             top = textGrob("Monosomes",gp=gpar(fontsize=20,font=3)))
dev.off()
...
png(filename = file.path(parent_dir, "plots/offset_aligned_single_nt_plots/all samples offset aligned single nt plot D30_21nts.png"), width = 1300, height = 300)
grid.arrange(UTR5_end_plot, CDS_start_plot, CDS_end_plot, UTR3_start_plot, nrow = 1, widths = c(1,2,2,1),
