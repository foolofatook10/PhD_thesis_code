...
extract_regions <- function(a){
  ...
  ENST <- unchanged[a,]$transcript 
  
  print(a)
  
  position <- unchanged[a,]$codon
...
test <- lapply(1:nrow(unchanged), extract_regions)

final_data <- do.call("rbind", test)

final_data <- final_data %>% filter(relative_position>= -60 & relative_position <= 60)
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/codons_deseq_codons_per_position_unchanged.png", res = 300, height = 1000, width = 1000)
