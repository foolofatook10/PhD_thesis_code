...
classes <- 
deseq %>% mutate(class = case_when(log2FoldChange > 0 ~ "Transcripts enriched in CNOT3 IP",
                                   log2FoldChange < 0 ~ "Transcripts depleted in CNOT3 IP")) %>% 
  select(transcript, class)
...
final <- inner_join(codons_n, total_codons, by = c("class", "bin"))
...
final %>%
  ...
  filter(wobble == "GC3") %>% 
  ggplot(aes(x = bin, y = percentage, colour = class)) +
  geom_point() +
  geom_line() +
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/CU_binned_deseq.png", res = 300, height = 1000, width = 1500)
print(CU_binned)
dev.off()
