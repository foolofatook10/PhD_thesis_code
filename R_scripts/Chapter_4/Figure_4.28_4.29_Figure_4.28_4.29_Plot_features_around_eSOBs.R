...
diffexor <- 
  codons_deseq %>% 
  mutate(class = factor(case_when(padj < 0.05 & log2FoldChange > 0 ~ "enriched",
                                  padj < 0.05 & log2FoldChange < 0 ~ "depleted",
                                  .default = "unchanged"),
                        levels = c("enriched", "unchanged", "depleted")))

enr <- diffexor %>% filter(class == "enriched")
unch <- diffexor %>% filter(class == "unchanged")
depl <- diffexor %>% filter(class == "depleted")

codon_level <- 
  diffexor %>% 
  ggplot(aes(x = log2FoldChange, y = -log10(padj), colour = class))+
  geom_point() +
  scale_colour_manual(values = c("#1B9E77", "#D95F02", "#7570B3"),
                      name = "Group",
                      labels = c(paste0("Enriched \n n = ", nrow(enr)),
                                 paste0("Unchanged \n n = ", nrow(unch)),
                                 paste0("Depleted \n n = ", nrow(depl)))) +
  ggtitle("Codon DE") +
  publication_theme() +
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/DE_codons_volcano_without_sustained_traces.png",
    res = 300, height = 1250, width = 1250)
print(codon_level)
dev.off()


enriched <- codons_deseq %>% filter(padj < 0.05 & log2FoldChange > 0)
...
#CNOT3_recruiting_codons <- c("cgg", "agg", "cga")

test <- lapply(1:nrow(enriched), extract_regions)

# is_CNOT3_recruiting_codon <- function(x){
#   
#   y <- 
#   x %>% 
#     filter(relative_position == 0) %>% 
#     pull(codon)
#   
#   tag <- ifelse(y %in% CNOT3_recruiting_codons, T, F)
#   
#   return(tag)
#   
# }
# 
# CNOT3_recruiting_codon_vec <- unlist(lapply(test, is_CNOT3_recruiting_codon))
# not_CNOT3_recruiting_codon_vec <- !CNOT3_recruiting_codon_vec
# 
# test2 <- test[not_CNOT3_recruiting_codon_vec]
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/wobble_without_polarised_transcripts.png",
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/codons_deseq_AAproperties_without_polarised_transcripts.png",
    res = 300, height = 1000, width = 2000)
print(AA_properties)
dev.off()
...
annotated_data$AA <- factor(annotated_data$AA,
                            levels = vec_of_AAs) 
...
g <- ggplot_gtable(ggplot_build(AA_frequencies))

strips <- which(grepl('strip-', g$layout$name))
#i = 1

colours <- c("#984EA3", "#984EA3", rep("#FF7F00", 3), rep("#4DAF4A", 4), "#984EA3",
             "#E41A1C", "#E41A1C","#E41A1C", rep("#377EB8", 2), 
             rep("#E41A1C", 5))

colours = c(rep("#E41A1C", 8), 
            rep("#377EB8", 2), 
            rep("#4DAF4A", 4), 
            rep("#984EA3", 3), 
            rep("#FF7F00", 3))
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/codons_deseq_AA_individual_without_polarised_transcripts.png",
    res = 300, height = 2000, width = 3000)
print(plot(g))
dev.off()
...
axis_label_x <- "Relative Codon Position\n(P site = 0)"
axis_label_y <- "Codon Frequency (%)"
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/ESOBs_aligned_codon_frequency_ProArgGluAsp_from_codon_deseq_without_sustained_trances.png",
    height = 3000, width = 2500, res = 300)
cowplot::plot_grid(Proline, Arg, Glu, Asp, ncol = 2, nrow = 2, align = "hv")
dev.off()

png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/codons_deseq_Proline_without_polarised_transcripts.png", res = 300, height = 1000, width = 1000)
print(Proline)
dev.off()



png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/codons_deseq_GLU_without_polarised_transcripts.png", res = 300, height = 1000, width = 1000)
print(Glu)
dev.off()
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/codons_deseq_ARG_without_polarised_transcripts.png", res = 300, height = 1000, width = 1000)
print(Arg)
dev.off()



png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/codons_deseq_Asp_without_polarised_transcripts.png", res = 300, height = 1000, width = 1000)
print(Asp)
dev.off()
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/codons_deseq_Lys_without_polarised_transcripts.png", res = 300, height = 1000, width = 1000)
print(Lys)
dev.off()
...
png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/codons_deseq_aligned_CPMS_enriched_without_polarised_transcripts.png", res = 300, height = 1000, width = 1000)
print(CPMs)
dev.off()

png("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/codons_deseq_codons_per_position_enriched_without_polarised_transcripts.png", res = 300, height = 1000, width = 1000)
