...
sample_info <- data.frame(row.names = RPF_sample_names,
                          condition = factor(rep(c("NTC", "siCNOT3", "siRPS25"), 5)),
                          replicate = factor(rep(c("1", "2", "3", "4", "5"), each = 3)))
...
ggplot(pcaData, aes(PC1, PC2, color=condition, label=replicate)) +
  ...
  theme(axis.title = element_text(size = 18),
        ...
        legend.title = element_blank(),
        plot.title = element_text(size = 20, face = "bold", hjust = 0.5))+
  ggtitle("siCNOT3/sieS25 RPFs") +
  publication_theme()
dev.off()

png(filename = file.path("\\\\data.beatson.gla.ac.uk/data/JETTLES/thesis_figures/riboseq_RPFs.png"), width = 1250, height = 1250, res = 300)
ggplot(pcaData, aes(PC1, PC2, color=condition, label=replicate)) +
  geom_point(size=3) +
  ggrepel::geom_text_repel(size = 6, show.legend = F)+
  scale_color_manual(values = c("#FFC20A", "#88CCEE", "#984EA3")) +
  xlab(paste0("PC1: ",percentVar[1],"% variance")) +
  ylab(paste0("PC2: ",percentVar[2],"% variance")) + 
  theme_bw()+
