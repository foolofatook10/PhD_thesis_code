...
sample_info <- data.frame(row.names = c(RPF_sample_names, Total_sample_names),
                          ...
                          SeqType = factor(c(rep("RPFs", 10), rep("RNA", 10))),
...
# #output summary
# sink(file = file.path(parent_dir, "Analysis/DESeq2_output", paste0("TE_", treatment, "_summary_siCNOT3.txt")))
# summary(res)
# sink()
...
write_csv(TE_output, file = file.path(parent_dir, "Analysis/DESeq2_output", paste0("FINAL_TE_", treatment, "_DEseq2_siCNOT3.csv")))




TE_output %>% filter(gene_sym == "CNOT1")


TE_output %>% filter(padj < 0.1)

TE_output %>% top_n(-100, padj)
